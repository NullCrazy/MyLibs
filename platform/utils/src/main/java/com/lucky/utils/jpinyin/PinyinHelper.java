...
public class PinyinHelper {
	...
	private static Properties mutilPintinTable = null;
	...
	private static Properties getPinyinTable(Context context){
		if (pinyinTable == null){
			pinyinTable = PinyinResource.getPinyinTable(context);
		}
		return pinyinTable;
	}

	private static Properties getMutilPintinTable(Context context){
		if (mutilPintinTable == null){
			mutilPintinTable = PinyinResource.getMutilPintinTable(context);
		}
		return mutilPintinTable;
	}
		...
		for (int i = 0, len = str.length(); i < len; i++) {
			...
			if (ChineseHelper.isChinese(c) || c == '〇') // 判断是否为汉字或者〇
			{
				...
				for (int rightIndex = (i + rightMove) < len ? (i + rightMove) : (len - 1); rightIndex > i; rightIndex--) {
					...
					if (getMutilPintinTable(context).containsKey(cizu)) {
						String[] pinyinArray = formatPinyin(getMutilPintinTable(context).getProperty(cizu), pinyinFormat);
