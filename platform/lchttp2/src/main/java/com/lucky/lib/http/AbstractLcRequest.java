...
import static com.lucky.lib.http.HttpCodeEnum.REQUEST_EXCEPTION;
...
public abstract class AbstractLcRequest implements IHttpRequest, IHttpParams {
     ...
     */
    private static final String LUCK_CACHE_FROM = "bcfg_cache_from";
    /**
     * 本地缓存
     */
     ...
     * 每个手机对应的UID
     */
    static final String UID = "Authorization";
    /**
     * 加密之后的数据
     */
...
//        }
    ...
    public IHttpRequest url(@NonNull String url) {
        if (!url.startsWith(URL_SUFFIX)) {
            throw new IllegalArgumentException("url要以反斜杠开始");
        }
        this.url = url;
        return this;
    }
    ...
    protected class InnerCallBack<T> implements Callback {
        ...
        public void onResponse(@NonNull Call call, @NonNull Response response) throws IOException {
            ...
            //判断返回结果中是否为数组
            boolean isArray = baseResponse != null && baseResponse.getData() instanceof JSONArray;
            ...
            if (isArray) {
                if (isList) {
                    ...
                    baseResponse.setData(JSONObject.parseArray(((JSONArray) baseResponse.getData()).toJSONString(), (Class<?>) lcNetCallBack.getEntityType()));
                } else {
            ...
            boolean isNullList = baseResponse.getData() == null && (lcNetCallBack.getEntityType() instanceof ParameterizedType || isList);
            if (isNullList) {
                baseResponse.setData(new ArrayList<>());
            }
            //处理api异常，上报monitor 数据
            if (baseResponse.getRtn() != 1) {
