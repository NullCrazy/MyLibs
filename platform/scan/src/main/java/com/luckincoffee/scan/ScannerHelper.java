...
import com.luckincoffee.scan.service.ScanService;
...
public class ScannerHelper implements InputManager.InputDeviceListener, ScanService.OnKeyEvent {
    ...
    public void register(OnScanSuccessListener onScanSuccessListener) {
        register(onScanSuccessListener, null);
    }
    ...
    public boolean isInputFromScanner(Context context, KeyEvent event) {
        ...
        if (event.getKeyCode() == KeyEvent.KEYCODE_UNKNOWN ||
                ...
                event.getKeyCode() == KeyEvent.KEYCODE_ALL_APPS ||
    ...
    private void execShell() {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    String cmd0 = "pm grant " + context.getPackageName() + " android.permission.WRITE_SECURE_SETTINGS";
                    Runtime.getRuntime().exec(new String[]{"/system/bin/su", "-c", cmd0});
                    SystemClock.sleep(2000);
                    Settings.Secure.putString(context.getContentResolver(), "enabled_accessibility_services", context.getPackageName() + "/" + ScanService.class.getCanonicalName());
                    Settings.Secure.putInt(context.getContentResolver(), "accessibility_enabled", 1);
                } catch (Exception var2) {
                    var2.printStackTrace();
                }
                try {
                    String cmd0 = "su pm grant " + context.getPackageName() + " android.permission.WRITE_SECURE_SETTINGS";
        ...
        try {
            if (!isAccessibilitySettingsOn(context)) {
                Settings.Secure.putString(context.getContentResolver(),
                        ...
                        context.getPackageName() + "/" + ScanService.class.getCanonicalName());
